...
- [x] Task: `config validate` Subcommand [da9178d]
    ...
    - [x] Implement `validate` subcommand in `cli.py`. The `validate` command: resolves project root and config file path, reads raw TOML (if file exists) for deprecation check via `check_deprecated_settings()`, attempts `load_config()` for schema validation (catch `ConfigError` → collect schema errors, friendly unknown-key message), if schema valid → call `validate_paths()` for path warnings, prints grouped findings (schema errors, path warnings, deprecation warnings) + summary, exit 0 if no schema errors or deprecated settings (path warnings are non-fatal), exit 1 if schema errors or deprecated settings present.
