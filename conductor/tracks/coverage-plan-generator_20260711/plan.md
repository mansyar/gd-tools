...
- [ ] Task: Implement `write_plan_json` serialization
    - [ ] Write tests for `write_plan_json` — valid plan → correct JSON structure (version, generated_by, files array with file_id, path, source_hash, lines)
    - [ ] Write test for `write_plan_json` — empty plan (no files) produces valid JSON with empty files array
...
- [ ] Task: Implement `read_plan_json` deserialization with error handling
    - [ ] Write test for `read_plan_json` — valid JSON file → correct `CoveragePlan` object
    - [ ] Write test for `read_plan_json` — missing file raises `CoveragePlanError`
    - [ ] Write test for `read_plan_json` — invalid JSON raises `CoveragePlanError`
    - [ ] Write test for `read_plan_json` — schema mismatch (wrong version, missing required fields) raises `CoveragePlanError`
    - [ ] Write test for `read_plan_json` — round-trip (write then read produces equal plan)
...
- [ ] Task: Implement `parse_gdscript` function
    ...
    - [ ] Write test for `parse_gdscript` — empty string returns valid tree
...
- [ ] Task: Implement `CoverageVisitor` — statement classification
    - [ ] Write test: `test_assignment_is_expr_stmt` — `var x = 5` produces an `expr_stmt` trackable point
    - [ ] Write test: `test_func_var_with_assignment_tracked` — `func_var_assigned`, `func_var_typed_assgnd`, `func_var_inf` all tracked
    - [ ] Write test: `test_declarations_not_tracked` — `const_stmt`, `class_var_stmt`, `signal_stmt`, `enum_stmt`, `func_var_empty`, `func_var_typed` NOT tracked
    - [ ] Write test: `test_empty_file_produces_empty_plan` — source with only declarations produces zero trackable points
...
- [ ] Task: Implement `CoverageVisitor` — branch classification
    - [ ] Write test: `test_if_else_produces_two_branch_entries` — if/else produces `if_true` and `if_false` branch points
    - [ ] Write test: `test_elif_produces_additional_branch` — elif adds `elif_true` branch point
    - [ ] Write test: `test_while_loop_body_tracked` — while produces `loop_body` branch point
    - [ ] Write test: `test_for_loop_body_tracked` — for and for_typed produce `loop_body` branch points
    - [ ] Write test: `test_match_each_case_tracked` — each match branch produces `match_case` branch point
    - [ ] Write test: `test_nested_control_flow` — nested branches produce correct branch types and unique IDs
    - [ ] Write test: `test_branch_ids_are_unique_per_file` — all IDs within a file are unique
...
- [ ] Task: Implement `generate_plan` function
    ...
    - [ ] Write test: `test_source_hash_computed` — each file entry has correct `sha256:` prefixed source hash
    ...
    - [ ] Implement `generate_plan(project_root, source_dirs, exclude_dirs, test_dirs) -> CoveragePlan` — reuses `discover_gd_files()`, filters test_dirs, parses each file, runs `CoverageVisitor`, assembles `CoveragePlan` with sequential `file_id`s
    - [ ] Verify tests pass and coverage >80% line / >70% branch
